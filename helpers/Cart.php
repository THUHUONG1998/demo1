...
class Cart
{
    ...
    public function add($product, $qty = 1)
    {
        ...
        // init luu thong tin mat hang can mua
        // truoc do chua mua san pham   
        $giohang = [
            'qty' => 0,
            'price' => 0, // don gia cua tong so gio hang
            'promotionPrice' => 0,
            'item' => null // thong tin san pham
        ];
        if (!empty($this->items) and array_key_exists($product->id, $this->items)) {
            // lay thong tin san pham da mua 
